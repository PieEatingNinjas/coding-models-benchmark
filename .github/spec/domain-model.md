...
| `Priority` | `Priority` | task priority (`Low`, `Medium`, `High`); defaults to `Medium` |
| `Secret` | `string?` | **internal; must never go out via the API** |

## Priority (enum — `src/TodoApi/Models/Priority.cs`)
Enumeration with values: `Low` (0), `Medium` (1), `High` (2).

## TodoItemDto (contract — `TodoItemDto.cs`)
Contains `Id`, `Name`, `IsComplete`, `Priority`. Has **no** `Secret` property — a compile-time guarantee that the field cannot leak.
